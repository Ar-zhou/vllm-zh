...
class DeepseekV32Model(torch.nn.Module, EagleModelMixin):
    ...
    def forward(
        ...
        aux_pp = get_pp_group().world_size > 1 and bool(self.aux_hidden_state_layers)
        remote_aux_hidden_states = (
            self.collect_remote_aux_hidden_states(intermediate_tensors)
            if aux_pp
            else []
        )
        aux_hidden_states = []
        if aux_pp and get_pp_group().is_first_rank and 0 in self.aux_hidden_state_layers:
            aux_hidden_states.append(hidden_states)
        for idx, layer in enumerate(
            islice(self.layers, self.start_layer, self.end_layer),
            start=self.start_layer,
        ):
            if not aux_pp and idx in self.aux_hidden_state_layers:
                aux_hidden_states.append(
                    hidden_states if residual is None else hidden_states + residual
                )
            hidden_states, residual = layer(positions, hidden_states, residual, attn_in)
            attn_in = None
            if aux_pp and idx + 1 in self.aux_hidden_state_layers:
