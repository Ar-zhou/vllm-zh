...
class PPHandler:
    ...
    consumed at step T+pp_size under async pipelining, or at the next step
    under synchronous PP+DSpark scheduling, via `get_prev_sampled_outputs`.
    ...
    def get_prev_sampled_outputs(
        ...
        """Consume the latest or delayed entry and wait for its recv event,
        then filter out entries whose request was freed since `receive`.
        """
        if not self.queue:
            return None
        if immediate:
            # A synchronous scheduler waits for the entire PP wave before it
            # schedules the next one. Its next decode therefore needs the most
            # recently broadcast sampled token and DSpark proposal, not the
            # pp_size-step-old entry used by async pipelining.
            slot = self.queue[-1]
            self.queue[-1] = None
        else:
