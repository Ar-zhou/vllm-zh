...
This is the the deployment host (8 × H800) deployment experiment. It uses the
...
tmux new-session -d -s glm52-v030-pp8 \
  'cd <checkout> && bash deploy/glm52_pp8_v030/start.sh'
...
environment at `the configured Python environment`; matching vLLM 0.30.0
...
`$repo_root/.runtime/`. The service runs in the foreground.
...
- Live session: `tmux attach -t glm52-v030-pp8`; log:
  `$repo_root/.runtime/logs/server-foreground-glm52-pp8-mtp3-v030-20260923-230243.log`.
