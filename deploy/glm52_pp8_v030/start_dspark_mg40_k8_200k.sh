...
: "${GLM52_DSPARK_TARGET:?Set GLM52_DSPARK_TARGET to the target checkpoint}"
model_path="$GLM52_DSPARK_TARGET"
...
log_path="$log_dir/server-foreground-glm52-pp8-dspark-mg40-k8-bf16-200k-v030-$(date +%Y%m%d-%H%M%S).log"
...
"$python_bin" -m vllm.entrypoints.openai.api_server \
  ...
  --speculative-config "{\"method\":\"dspark\",\"model\":\"$draft_path\",\"num_speculative_tokens\":8,\"draft_sample_method\":\"greedy\",\"attention_backend\":\"FLASH_ATTN\",\"kv_cache_dtype\":\"bfloat16\",\"enable_adaptive_verification\":false}" \
